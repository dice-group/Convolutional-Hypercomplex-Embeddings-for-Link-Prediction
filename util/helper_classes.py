...
class HeadAndRelationBatchLoader(torch.utils.data.Dataset):
    def __init__(self, er_vocab, num_e, device):
        self.device = device
        self.num_e = num_e
        head_rel_idx = torch.Tensor(list(er_vocab.keys())).long()
        self.head_idx = head_rel_idx[:, 0].to(device=device)
        self.rel_idx = head_rel_idx[:, 1].to(device=device)
    ...
    def __getitem__(self, idx):
        y_vec = torch.zeros(self.num_e, device=self.device)
...
class Reproduce:
    ...
    def evaluate_link_prediction_range_constraint(self, model, data, range_of_relations, per_rel_flag_=True):
        hits = []
        ranks = []
        ...
        set_of_entities = set(range(len(self.dataset.entities)))
        for i in range(0, len(test_data_idxs), self.batch_size):
            data_batch, _ = self.get_batch_1_to_N(er_vocab, test_data_idxs, i)
            ...
            for j in range(data_batch.shape[0]):
                filt = er_vocab[(data_batch[j][0], data_batch[j][1])]
                filt = list(set(filt + list(set_of_entities - set(range_of_relations[r_idx[j].item()]))))
                target_value = predictions[j, e2_idx[j]].item()
                predictions[j, filt] = 0.0
                predictions[j, e2_idx[j]] = target_value
            sort_values, sort_idxs = torch.sort(predictions, dim=1, descending=True)
    ...
    def reproduce(self, model_path, data_path, model_name, apply_range_constraint=False, per_rel_flag_=False,
                  tail_pred_constraint=False,
                  out_of_vocab_flag=False, batch_size=1024):
        ...
        self.batch_size = batch_size  # self.kwargs['batch_size']
        print('Link Prediction Results on Testing')
        if apply_range_constraint:
            range_of_relations = dict()
            for (e1, p, e2) in self.dataset.train_data:
                range_of_relations.setdefault(self.relation_idxs[p], list()).append(self.entity_idxs[e2])
            self.evaluate_link_prediction_range_constraint(model, self.dataset.test_data, range_of_relations,
                                                           per_rel_flag_)
